...
class StdOutListener(StreamListener):
    def on_data(self, data):  # pulling the text out of the JSON file each tweet comes in
        ...
        try:
            ...
            cur.execute('insert into twitter_stream_ITV (datetime_created, user_id, user_name, user_screen_name,\
...
stream.filter(track=['ITV', '@itv'])
tweet_sample = stream.filter(track=['ITV', '@itv'])